...
自分は普段、自宅 PC とオフィス PC を VPN で繋ぎながら開発しています。そのとき、片方の PC で進めた作業を、もう一方の PC で続けようとすると、エージェントはそれまでの文脈を一切覚えていません。同じ会話を最初からやり直すような状態になり、地味につらい時間が積み上がっていきました。

加えて、自分自身も「いつ、どのタイミングで、なぜその設計判断をしたか」を覚えきれない、というのも痛みでした。チャット履歴を遡れば書いてはあるのですが、検索性は弱く、別マシンや別ツールに移ると拾えなくなります。

もう一つ感じていたのが、複数エージェントで「コーディング担当」と「レビュー担当」を分けて動かすときの摩擦です。サブのエージェントが毎回フォルダ全体を読みに行き、現状把握が終わるまで本題に入れず、なかなか作業が進みません。事前に文脈を渡せれば早いのに、と何度も思っていました。

既存の AI エージェント向け長期記憶ツールはいくつかあります。ただ、SaaS にデータを預けることが前提だったり、逆に1台の中だけで閉じる前提だったりして、「自分の手元の複数台で、自分のデータのまま、LAN/VPN に閉じたまま共有する」というニーズには微妙に届きませんでした。それなら自分で作ろう、というのが kioku-mesh のスタートです。
