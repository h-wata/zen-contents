...
自分は普段、自宅 PC とオフィス PC を 行き来しながら開発しています。そのとき、片方の PC で進めた作業を、もう一方の PC で続けようとすると、エージェントはそれまでのContextを覚えていません。同じ会話を最初からやり直すような状態で待ち時間が苦痛でした。

加えて、自分自身も「いつ、どのタイミングで、なぜその設計判断をしたか」を覚えきれない、というのも痛みでした。チャット履歴を遡れば書いてはあるのですが、検索性は弱く、別マシンや別ツールに移ると拾えなくなります。

もう一つ辛かったのが、複数エージェントで「コーディング担当」と「レビュー担当」を分けて動かすときです。サブのエージェントが毎回フォルダ全体を読みに行き、現状把握が終わるまで本題に入れず、なかなか作業が進みません。
