...
kioku-mesh のデフォルトは、信頼できるネットワークの上で動かす前提の設計です。閉じた LAN・Tailscale・WireGuard などで `7447/tcp` を絞ることが前提で、それで多くの個人メッシュは十分です。

ただし次のような状況では、ネットワーク層の信頼だけでは足りません。

- Tailscale ACL の設定ミスで、本来繋いではいけないノードが届くようになっていた
- 同居人や同僚と LAN を共有している
- VPN を解約・更新したタイミングで、一時的に「信頼できるネットワーク」の前提が崩れる
...
- 連載第4回: [kioku-mesh で hub を立てて spoke を繋ぎ、マシン横断の共有メモリを作る](https://zenn.dev/h_wata/articles/kioku-mesh-04-hub-spoke)
